...
func (s *UserService) ClearUserSessionContext(userId string, msg string) bool {
	if strings.Contains(msg, "下个问题") || strings.Contains(msg, "我要问下个问题") {
...
func NewUserService() UserServiceInterface {
	...
	return &UserService{cache: cache.New(time.Second*config.LoadConfig().SessionTimeout, time.Minute*1)}
}
